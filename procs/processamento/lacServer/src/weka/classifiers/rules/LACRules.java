...
public class LACRules implements Serializable
{
	 ...
	 * @throws Exception 
	 */
	public double[] calculateProbabilities(LACInstance testInstance) throws Exception
	{
		double[] probs;
	...
	private double[] calculateScores(LACInstance testInstance) throws Exception
	{
		...
		testInstance.setRules((ArrayList<LACRule>) allRulesForFeatures);
		if (numRules > 0)
		{
	...
	{
		numPatterns[0]++;
		LACRule[] rules = getRules(pattern);
		if(rules != null && rules.length > 0)
		{
			extractedRules.addAll(Arrays.asList(rules));
	...
	private LACRule[] getRules(List<Integer> featuresCombination)
	{
		LACRule[] rulesForFeatures = cache.getRules(featuresCombination);
	 ...
	 * @param featuresCombination
	 */
	private LACRule[] doExtractRules(List<Integer> featuresCombination)
	{
		...
		if (size > 0)
		{
			...
			for (int i = 0; i < numClasses; i++)
			{
				...
				if (classCount > 0)
				{
					float support = (float) classCount / (float) trainingSet.length();
					float confidence = (float) classCount / (float) size;
		...
		return (LACRule[]) rules.toArray(new LACRule[rules.size()]);
	}
	
	public LACInstances getTrainingInstances(){
		return trainingSet;
	}
}